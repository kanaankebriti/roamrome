...
{
	unsigned char i = 0;						// selected index for hud_tiles and also loop counter
	unsigned char tile_1_indx = 0;				// tile 1 selected index for tile_set
	unsigned char tile_2_indx = 0;				// tile 2 selected index for tile_set
	...
	bool pair_of_tiles_swap_toggle = false;		// swap allowance state for two tiles
	...
	unsigned char lvl_num = 15;					// start by loading lvl 1
	...
	while (SDL_WaitEvent(&event))
		...
		case SDL_MOUSEBUTTONDOWN:
			...
			// retrive selected tile_set index for tile 1 or tile 2 based on toggle variable
			if (pair_of_tiles_swap_toggle)
				tile_2_indx = get_selected_tile_index(tile_set, lvl_num_of_tiles, mouse_pos_x, mouse_pos_y);
			else
				tile_1_indx = get_selected_tile_index(tile_set, lvl_num_of_tiles, mouse_pos_x, mouse_pos_y);

			// check for get_selected_tile_index out of bound
			if (tile_1_indx == lvl_num_of_tiles)
				pair_of_tiles_swap_toggle = false;
			// check for first selected tile
			else if (!pair_of_tiles_swap_toggle)
				pair_of_tiles_swap_toggle = true;	// on next click check for tile 2 (index j)
			// check for secound selected tile out of bound
			else if (tile_2_indx == lvl_num_of_tiles)
				pair_of_tiles_swap_toggle = false;	// revoke swap toggle
			// one tile has been selected
			else
			{
				// swap tiles
				swap_two_tiles(&tile_set[tile_1_indx], &tile_set[tile_2_indx]);

				// reset swap toggle variable
				pair_of_tiles_swap_toggle = false;

				SDL_RenderClear(renderer);
				render_still_layer_with_hud(renderer, tile_set, lvl_num_of_tiles, texture_bank, hud_tiles, hud_num_of_tiles);
			}
